...
class Human:
    ...
    def days_indexes(self):
        ...
        print(f"Gladness = {self.gladness} proportion of happiness")
        print(f"Car brand = {self.brand}")
        print(f"Size of fuel in the car = {self.fuel}")
        print(f"Your car is running if it shows more than zero = {self.strength}")
        print(f"Your home freshness proportion = {self.home.mess}")
        print(f"Size of food in the house = {self.home.food}")
    def is_alive(self):
        if self.satiety <= 0:
            self.alive = False
            print("You were so hungry that you ate too many pills and overdosed")
        elif self.gladness <= 0:
            self.alive = False
            print("You are depressed")
        elif self.money <= -300:
            self.alive = False
            print("Now you're a bum")
